...
export default function ServicesPage() {
  ...
  const handleConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    setBookingConfirmedCode(`SCPK-${Math.floor(10000 + Math.random() * 90000)}`);
  };
            ...
            {!bookingConfirmedCode ? (
              <form onSubmit={handleConfirm} className="space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block font-semibold mb-1">Your Street Address</label>
                    <input
                      type="text"
                      required
                      placeholder="Street, Sector, Area Name"
                      className="w-full p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800"
                    />
                  </div>
                  <div>
                    <label className="block font-semibold mb-1">Phone Number for WhatsApp</label>
                    <input
                      type="text"
                      required
              ...
              <div className="text-center space-y-3 py-2">
                ...
                <p className="text-xs text-slate-500">Booking Code: <span className="font-mono font-bold text-emerald-500">{bookingConfirmedCode}</span></p>
                <button
                  onClick={() => setSelectedService(null)}
