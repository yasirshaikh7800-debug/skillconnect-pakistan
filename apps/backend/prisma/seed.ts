import { PrismaClient, UserRole, UserStatus, BookingStatus, PaymentMethod, PaymentStatus } from '@prisma/client';
...
function slugify(value: string | number | boolean) {
  return String(value)
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]+/g, '-')
...
async function main() {
  ...
  const provinceRecords = [] as Array<{ id: string; slug: string }>;
  ...
  const provinceBySlug = new Map<string, string>(provinceRecords.map((item) => [item.slug, item.id]));
  ...
  const cityBySlug = new Map<string, SeedCity>(allCities.map((city: any) => [city.slug, city]));
  ...
  const createdCustomers = [] as Array<any>;
  ...
  for (const category of categories) {
    ...
    if (record) serviceCategoryBySlug.set(category.slug, record.id);
  }
  ...
  const createdProviders = [] as Array<any>;
  ...
  for (const company of companyData) {
    ...
    if (!owner || !category || !city) continue;
  ...
  const createdCompanyLookup = new Map<string, string>();
  const companies = await prisma.company.findMany({ select: { slug: true, id: true, ownerId: true } });
  companies.forEach((company: SeedCompany) => createdCompanyLookup.set(company.ownerId, company.id));

  const serviceTemplates: Record<string, Array<[string, string, number]>> = {
    'electrical-wiring': [
  ...
  const providerUserIds = await prisma.user.findMany({ where: { role: UserRole.PROVIDER }, select: { id: true, providerProfile: true } });
  for (const [index, providerUser] of providerUserIds.entries()) {
    ...
    const templates = serviceTemplates[providerMeta.categorySlug as keyof typeof serviceTemplates] ?? [];
  ...
  const customersList = await prisma.user.findMany({ where: { role: UserRole.CUSTOMER }, select: { id: true } });
  const providersList = await prisma.user.findMany({ where: { role: UserRole.PROVIDER }, select: { id: true } });
  ...
  for (let index = 0; index < Math.min(45, services.length); index++) {
    ...
    const booking = await prisma.booking.create({
      data: {
        ...
        status: index % 3 === 0 ? BookingStatus.COMPLETED : BookingStatus.ACCEPTED,
